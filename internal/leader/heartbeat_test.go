...
func TestLeaderHeartbeatRegistersAgent(t *testing.T) {
	...
	leader.Heartbeat("remote-agent", "http://192.168.1.10:8080", nil, nil, time.Time{}, "")
...
func TestLeaderHeartbeatLearnsJobsFromRemoteAgents(t *testing.T) {
	...
	leader.Heartbeat("remote-agent", "http://192.168.1.10:8080", remoteJobs, taskCounts(remoteJobs), time.Now(), "")
	time.Sleep(10 * time.Millisecond)
...
func TestLeaderHeartbeatSyncsNewerState(t *testing.T) {
	...
	leader.Heartbeat("remote-agent", "http://192.168.1.10:8080", remoteJobs, taskCounts(remoteJobs), newerTime, "")
	time.Sleep(10 * time.Millisecond)
...
func TestLeaderGetAgents(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		leader.Heartbeat("agent-"+string(rune('a'+i)), "http://host:8080", nil, nil, time.Time{}, "")
	}
...
func TestLeaderConcurrentHeartbeats(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func(n int) {
			...
			for j := 0; j < 10; j++ {
				leader.Heartbeat(agentID, "http://host:8080", nil, nil, time.Time{}, "")
			}
		}(i)
	}
