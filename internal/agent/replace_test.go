...
func TestHandleRunReplaceVervangEigenTaakBinnenVolleNode(t *testing.T) {
	...
	run := func(job types.Job, replace bool) *httptest.ResponseRecorder {
	...
	if w := run(types.Job{Name: "welcome", Command: "v1", MemoryLimit: 600 << 20}, false); w.Code != http.StatusAccepted {
		t.Fatalf("welcome v1: status %d", w.Code)
	}
	if w := run(types.Job{Name: "buurman", Command: "b", MemoryLimit: 400 << 20}, false); w.Code != http.StatusAccepted {
	...
	if w := run(types.Job{Name: "welcome", Command: "v2", MemoryLimit: 600 << 20}, false); w.Code != http.StatusServiceUnavailable {
	...
	if w := run(types.Job{Name: "welcome", Command: "v2", MemoryLimit: 600 << 20}, true); w.Code != http.StatusAccepted {
	...
	if w := run(types.Job{Name: "welcome", Command: "v3", MemoryLimit: 700 << 20}, true); w.Code != http.StatusServiceUnavailable {
